...
function Post() {
    ...
    async function toDeletePost(id) {
        let isDelete = window.confirm("Ви впевнені, що хочете видалити цей пост?");
        if(isDelete) {
    ...
    return(
        <mainDiv>
            ...
            <select value={toSort} onChange={(e) => sortFunction(e.target.value)} class = "sortSelect" name="sort" id="select">
                <option selected value="dis" disabled>Сортування постів</option>
                <option id = "New" value="-">Від нових до старих</option>
                <option id = "Old" value="+">Від старих до нових</option>
            </select>
            <br/>
            ...
            <option selected value='Усі'>Усі</option>
                {
